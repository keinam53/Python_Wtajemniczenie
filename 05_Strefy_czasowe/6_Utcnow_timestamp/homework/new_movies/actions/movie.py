...
from new_movies.exceptions import NoCreditsForMovieRent, MovieNotFound, ViewsLimitReached, TooYoungForMovie
...
def add_movie(user):
    ...
    print(f'Dodano film: {new_movie.info_with_date_format(date_and_time_format)}')


def rent_movie(user, movie):
    if user.credits_left < 1:
        raise NoCreditsForMovieRent()
    if not movie.is_appropriate_to_watch(user.age):
