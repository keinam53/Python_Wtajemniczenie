...
def test_normal_promotion_policy_pass(passing_grade, failing_grade):
...
def test_normal_promotion_policy_fail(passing_grade, failing_grade):
...
def test_strict_promotion_policy_pass(passing_grade, failing_grade):
...
def test_strict_promotion_policy_fail_grades(passing_grade, failing_grade):
    final_grades = [failing_grade, failing_grade, failing_grade, passing_grade]

    assert GradeCalculator.strict_promotion_policy(final_grades) is False


def test_strict_promotion_policy_fail_avg(poor_grade, failing_grade):
    final_grades = [failing_grade, poor_grade, poor_grade]

    assert GradeCalculator.strict_promotion_policy(final_grades) is False


def test_get_number_of_failing_grades_return_fail(passing_grade, failing_grade):
    final_grades = [failing_grade, passing_grade, failing_grade]

    assert GradeCalculator.get_number_of_failing_grades(final_grades) == 2


def test_get_number_of_failing_grades_empty_list():
    final_grades = []
